...
class UpdateState extends State<Update>{
  ...
  void update() async{
    try{
      final obj  = {
        "id" : todo['id'],
        "title" : titleCont.text, // 수정할 제목은 입력사자에서
        "content" : contentCont.text, // 수정할 내용은 입력상자에서
        "done" : todo['done']
      };

      final response = await dio.put("http://192.168.40.235:8080/api/todo", data:obj );
      final data = await response.data; print(data);
      if( data != null ){ Navigator.pop(context , true); }
      // 만약 수정 성공시 뒤로가기, 즉] 현재 위젯을 제거하면서 두번째 매개변수로 true 반환
    }catch(e){print(e);}
  }
